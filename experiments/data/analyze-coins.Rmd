...
plot ig versus eig:
```{r}
p1 = qplot(data = di, x = eig, y = ig, size = I(3), alpha = I(0.5)) + xlim(0,0.7) + ylim(0,0.7) + geom_abline()
```

direct labeled version:
```{r}
direct.label(qplot(data = di, x = eig, y = ig, size = I(3), color = experiment) + xlim(0,0.7) + ylim(0,0.7) + geom_abline())
```

(note that direct label jitters)
...
```{r}
qplot(data = di, x = eig.ignorance, y = ig, size = I(3), alpha = I(0.5)) + xlim(0,0.7) + ylim(0,0.7) + geom_abline()
```

direct labeled version:
```{r}
direct.label(qplot(data = di, x = eig.ignorance, y = ig, size = I(3), color = experiment) + xlim(0,0.7) + ylim(0,0.7) + geom_abline())
```

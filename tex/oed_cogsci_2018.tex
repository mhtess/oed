...
With our approach, the scientist writes their models as probabilistic programs, defines a space of possible experiments and results, and hands these to OED for experiment selection.
We stress that the tool \emph{complements} scientists; it does not replace them.
Our tool merely eliminates the need to manually comb large spaces of potential experiments, which is time consuming, tedious, and prone to bias, such as a preference for qualitative differences at the expense of ultimate quantitative information gain.

...
Good experiments are ones where the models make different predictions, but there is typically a large space of possible experiments one could run (e.g., there could many different possible stimulus sets to present).
...
We want to compare two cognitive models of people's behavior: $m_{\text{fair}}$, where people believe the coin is unbiased (i.e.,  \lstinline{H} and  \lstinline{T} are equally likely), and $m_{\text{bias}}$, where people believe the coin has a bias that is unknown  to them.
...
We wish to update our beliefs about these models through an experiment where we show people 4 flips of the same coin and ask them to predict what will happen on the next flip.
...
Under $m_{\text{fair}}$, $p(\texttt{H}) = \frac{1}{2}$ but under $m_{\text{bias}}$, $p(\texttt{H}) = \frac{5}{6}$ (this is the updated probability of heads after estimating the coin bias).
...
WebPPL supports sampling from primitive probability distributions and combining these samples in various ways, e.g., adding Gaussian noise to a Binomial random variable:
...
Infer(g)
\end{lstlisting}
The function \lstinline{g} defines a sampling procedure for our compound distribution.
This \emph{implicitly} represents a probability distribution; to reify this into an actual distribution, we must perform inference via \texttt{Infer(g, options)}.
...
For instance, in the model above, we might ask what values of $x$ and $y$ could lead their sum to be greater than or equal to 2:
...
Here, we consider three simple models of people's beliefs: (a) \emph{Fair coin}: people assume the coin is fair, (b) \emph{Bias coin}: people believe the coin has some unknown bias that they can estimate from data (i.e., learning the probability of an \lstinline{H} outcome), (c) \emph{Markov coin}: people believe the coin has some probability of transitioning between spans of \lstinline{H} and \lstinline{T} outcomes, also learnable from observations.
...
Several experiments have 0 information gain (e.g., \lstinline{HTHT})---the models make exactly the same predictions in this case (albeit for different reasons, as discussed earlier), so the experiment has no distinguishing power.
...
\begin{figure}[t]
            (a) \\ \includegraphics[width=0.9\columnwidth]{img/coin_predictions.pdf}\\ (b) \\ \includegraphics[width=0.9\columnwidth]{img/coin_eig_3way_nsubj_wlegend.pdf}
...
\begin{figure}[t]
  \includegraphics[width=0.85\columnwidth]{img/category-model-diffs.pdf}
  \caption{In MS and optimal experiments, the divergence between model predictions (i.e., the absolute value of the difference in probabilities of being classified as an A by the two models). The models diverge more in the optimal experiment.}
...
\begin{figure}[h]
(a) \\
\includegraphics[width=0.85\columnwidth]{img/category-eig-dist.pdf}\\
(b) \\
\includegraphics[width=0.85\columnwidth]{img/category-aig-curve.pdf}
...
This approach is compatible with certain challenging features of cognitive science experiments: participants give noisy responses, experimental results are sensitive to sample size, and models often require \emph{linking functions} to convert model output into predictions about experimental data.

...
For each experiment $x$, we computed the expected information gain from running our empirical sample of participants\footnote{\,N's are uneven due to randomization. We use the empirical N's for expected information gain in comparisons to actual information gain.} and compared this to the actual information gained from the empirical data, $\dkl{P(M \mid Y = y_\text{actual}, X = x)}{P(M)}$. We did this for the three model comparison scenarios described earlier.

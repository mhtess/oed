...
However, while the mathematical foundations of OED are fairly straightforward \cite{Lindley1956}, it has not enjoyed widespread use in practice.
...
Approximated OED can still be useful, as it can it find good experiments that we would not have discovered by hand.
...
(Indeed, this is the implicit assumption behind standard data analysis used for such models.)
...
In particular, we use WebPPL (\url{webppl.org}), a small but feature-rich probabilistic programming language embedded in Javascript \cite{dippl}.
...
\textbf{TODO: give high level summary of how webppl-oed works.}
...
OED({
  ...
  Y: function() { return randomInteger(n + 1) }
})
\end{lstlisting}
...
For each experiment $x$, we computed the expected information gain from running our empirical sample size of participants\footnote{N's are uneven due to randomization. We use the empirical N's for expected information gain in comparisons to actual information gain.} and compared this to the actual information gained from the empirical data, $\dkl{P(M \mid Y = y_\text{actual}, X = x)}{P(M)}$. We did this for the three model comparison scenarios described earlier.
...
\textbf{TODO in addition, the models are non-bayesian.}
...
\textbf{handle parameter inference}

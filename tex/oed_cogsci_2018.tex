...
We want to compare two models: $m_{\text{fair}}$, in which people believe the coin is unbiased (i.e.,  \lstinline{H} and  \lstinline{T} are equally likely), and $m_{\text{bias}}$, in which people believe the coin has a bias that is unknown  but that can be learned from data.
...
We wish to update this belief distribution over models by conducting an experiment where we show people four flips of the same coin and ask them to predict what will happen on the next flip.
There are 16 possible \emph{experiments} (sequences of \lstinline{H} and \lstinline{T} for the 4 flips that we show to participants); as each participant responds by predicting either \lstinline{H} or \lstinline{T}, there are $2^n$ possible \emph{outcomes} for $n$ participants.
Each model predicts how people would respond after seeing some particular sequence of flips (experiment).
...
\mht{this is a meaty paragraph.. consider streamlining or breaking into 2}
...
With our approach, the scientist writes her models as probabilistic programs, defines a space of possible experiments and results, and hands these to OED for experiment selection.
We stress that the tool \emph{complements} scientists; it does not replace them.
Our tool merely eliminates the need to manually consider large spaces of potential experiments, which can be time consuming, tedious, and prone to bias, such as a preference for qualitative differences at the expense of ultimate quantitative information gain.
...
First, we have examined model comparison problems where there are a finite number of models.
...
Second, we have restricted attention to ``one-shot'' experiments, but it would be useful to extend our work to sequential settings such as adaptive testing.

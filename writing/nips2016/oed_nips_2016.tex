...
In the full (fair--bias--Markov) comparison, the worst experiments (e.g., \lstinline{TTHH} ) are again cases where all models make similar predictions.
...
The second best experiment, \lstinline{HHHT}, is much better at distinguishing the bias model from the Markov model (because it predicts a qualitative difference; Figure \ref{fig:coin_preds}a), but the actual information gain is predicted to be less.
...
The expected information gain of an experiment depends on the the sample size of participants in that experiment.
Figure \ref{fig:coin_preds}b shows how, in the full comparison, the expected information gain for the top three experiments varies as a function of sample size.
This function is non-linear and the rank ordering of experiments changes with more participants---\lstinline{HTHT} and \lstinline{HHHT} switch places at around 12 participants.
...
\begin{figure}[t]
  ...
  \end{tabular}
  \caption{(a) Model predictions for the optimal (HHH) and two other experiments. (b) Expected information gain for HHHH, HHHT, and HTHT as a function of sample size (three-way model comparison).}
...
We randomly assigned 351 participants to a single experiment (all of the 16 experiments were completed by $\geq$20 unique participants).

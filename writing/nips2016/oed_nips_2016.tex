...
\caption{Running OED in the subjective randomness domain \mht{this is with the number of subjects that we collected; might want to make this for n = 1 (or n = 20) to highlight the symmetries}}
...
Figure TODO shows that expected information gain is a reliable predictor of the empirical value of an experiment (minimum $r$ = 0.857).
...
Indeed, the MS experiment only the \red{N}th best experiment (Figure~\ref{fig:dist})
...
Figure~\ref{fig:dist} shows that, while both experiments give maximal actual information gain with 40 participants, the optimal experiment reaches this maximum with only 8 participants, whereas the MS experiment requires 20.

...
Minimally, you must have a hypothesis in hand, a space of possible experiments, and clarity to reason through the logic of your hypothesis in each possible experiment. 
Psychology experiments have a number of additional complications. 
For instance, data costs money and deciding on the number of participants for a study is another crucial decision variable. 
...
We present a general, turn-key approach to designing experiments that best disambiguate competing hypotheses using a Bayesian framework. 
...
We are not the first to attempt to develop a framework for optimal experiments in psychology. Previous attempts suffer from a number of pragmatic issues that impede the psychologist who wishes to readily apply optimal experiment design (OED) to their research.
...
and a lack of analysis in dealing with practical experimental concerns in psychology, including dealing with noisy responses from participants, determining ideal numbers of participants, and deciding upon appropriate dependent measures (i.e., linking functions).

Doing the standard, active learning thing with probabilistic programs. 

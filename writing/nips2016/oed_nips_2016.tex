...
We present a general and principled framework to designing experiments that uses a Bayesian approach to find experiments that best distinguish competing hypotheses.
For a system to be domain-general, the computation for experiment selection must be automatic.
This is only possible with a common formalism for specifying hypotheses and experiments.
\emph{Probabilisitic programming languages} (PPLs) are such a formalism; they are high-level and universal languages for expressing  probabilisitic models.
...
We apply our framework to the domain of psychology, which is increasingly interested in experiment design and analysis. \red{(cite some replication thing?})
Studying human behavior introduces additional complications into experiment design.
Human data is noisy and sensitive to the dependent measure of the task.
Further, human data is expensive: The scientist must balance expected statistical power with the constraints of a limited budget to decide on the number of participants to run.
...
In this paper, we introduce a general framework for optimal experiment design that is able to deal with specific issues from psychology: noisy responses, determination of sample size, and in principle, selection among dependent measures or linking functions for the task at hand.
We construct a Bayesian model selection framework implemented in a practical modeling system.

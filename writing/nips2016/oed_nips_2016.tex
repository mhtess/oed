...
\begin{lstlisting}[mathescape, label={code:oed-pp}, caption = OED implementation (TODO: note that this is a little bit idealized version of actual code)]
var OED = function(mSample, xSample, ySample, infer) {
  ...
  infer.X(function() {
    ...
    // precompute $p(y) = \sum\limits_{m} p_m(y \mid x)p(m)$
    var yLikelihood = infer.Y1(function() {
      var y = ySample();
      var m = sample(mPrior);
      var mx = m(x);
      factor(score(mx, y));
    ...
    // $p(x) \sum\limits_{y} p(y) \dkl{ P(M \mid Y = y) }{ P(M) }$
    ...
    factor(eig); // convert eig to a probability?
                 // e.g., prob of observing the posterior distribution given the prior?
    return {x: x, EIG: eig}
  })
}

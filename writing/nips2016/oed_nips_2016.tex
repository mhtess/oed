...
Formal, matehmatical models alleviate some of these issues.
...
\ndg{need some cites to previous OED type work in this section...}
While the mathematical formulation of OED is relatively straightforward, in information theoretic terms, there has been a key obstacle to applying OED in practice: a complex calculation is required to formulate the objective for each new set of models and experiments. 
This can be sufficiently difficult and time-consuming to deter working scientists (particularly in the behavioral sciences) from OED. 
...
To actually obtain samples, we must perform inference on this model using \lstinline{Infer(model, options)} and turn it into a distribution.
